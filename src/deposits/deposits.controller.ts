import { Body, Controller, Get, Param, Post, Query, UseGuards } from '@nestjs/common';
import { ApiBearerAuth, ApiExcludeEndpoint, ApiOperation, ApiTags } from '@nestjs/swagger';
import { JwtAuthGuard } from '../common/guards/jwt-auth.guard';
import { CurrentUser } from '../common/decorators/current-user.decorator';
import type { AuthenticatedUser } from '../auth/interfaces/authenticated-user.interface';
import { DepositsService } from './deposits.service';
import { InitiateDepositDto } from './dto/initiate-deposit.dto';

@ApiTags('deposits')
@Controller('deposits')
export class DepositsController {
  constructor(private readonly depositsService: DepositsService) {}

  @Post()
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Initiate a deposit; returns a Paystack checkout URL. Wallet is credited only after webhook confirmation.' })
  initiate(@CurrentUser() user: AuthenticatedUser, @Body() dto: InitiateDepositDto) {
    return this.depositsService.initiate(user.userId, dto.amount);
  }

  /**
   * Where Paystack's checkout page redirects the user's browser after payment.
   * Unauthenticated (Paystack sends no JWT) and purely informational - the wallet
   * is only ever credited by the server-to-server webhook, never by this redirect.
   */
  @Get('callback')
  @ApiExcludeEndpoint()
  callback(@Query('reference') reference?: string, @Query('trxref') trxref?: string) {
    return {
      message:
        'Payment received. Your wallet will be credited once Paystack confirms the transaction via webhook - check GET /deposits/:reference for status.',
      reference: reference ?? trxref ?? null,
    };
  }

  @Get(':reference')
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Check the status of a deposit by reference' })
  getStatus(@CurrentUser() user: AuthenticatedUser, @Param('reference') reference: string) {
    return this.depositsService.getStatus(reference, user.userId);
  }
}
